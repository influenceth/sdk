...
use influence::components::{Deposit, Extractor, Processor, Building, Delivery};
use influence::types::{Entity, EntityTrait, InventoryItem};
use influence::systems::missions::starter::{key, building_fingerprint, hash};
use cubit::f64::Fixed;
...
fn sdk_binding_fixtures() {
    ...
    let process_hash = dump('process', @processor);
    assert(process_hash == hash(@processor), 'process mismatch');
    dump('process_changed', @Processor { finish_time: 1700001004, ..processor });
